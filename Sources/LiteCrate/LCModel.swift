...
extension LCModel {
  ...
  internal static func tableUpdatedPublisher(in crate: LiteCrate, notifyOn queue: DispatchQueue = DispatchQueue.main) -> AnyPublisher<Void, Never> {
  ...
  public static func publisher(in crate: LiteCrate, forAllWhere sqlWhereClause: String? = nil, values: [Any]? = nil)
  -> AnyPublisher<[Self], Never> where Self: LCModel
  {
    Just(())
      ...
      .map { _ in (try? Self.fetchAll(from: crate, forAllWhere: sqlWhereClause, values: values)) ?? [] }
      .eraseToAnyPublisher()
  }
  
  public static func publisher(in crate: LiteCrate, forPrimaryKey primaryKey: ID) -> AnyPublisher<Self?, Never> {
    Just(())
      .receive(on: crate.updateQueue)
      .append(Self.tableUpdatedPublisher(in: crate))
      .map {
  ...
  public static func fetchAll(from crate: LiteCrate, forAllWhere sqlWhereClause: String? = nil, values: [Any]? = nil)
  throws -> [Self]
  {
